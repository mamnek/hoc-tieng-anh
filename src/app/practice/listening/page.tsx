...
import { Trophy, Coins, X, Volume2, ArrowRight } from 'lucide-react';
...
function ListeningContent() {
  ...
  useEffect(() => {
    if (gameState === 'playing' && !feedback && questions[currentIndex]) {
      inputRef.current?.focus();
      speakWord(questions[currentIndex].term);
    }
  }, [currentIndex, gameState, feedback, questions]);
  ...
  const finishGame = () => {
    ...
    const coins = calculateCoins('listening', accuracy, questions.length);
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-4 max-w-2xl mx-auto w-full gap-8">
        <div className="text-center mb-4">
          <h2 className="text-xl font-semibold text-gray-600 mb-8">Nghe và viết lại từ</h2>
          <button 
            onClick={() => speakWord(currentQ.term)}
            className="w-32 h-32 bg-blue-600 hover:bg-blue-700 text-white rounded-full flex items-center justify-center mx-auto shadow-lg transition-transform hover:scale-105 active:scale-95"
          >
        ...
        {feedback === 'wrong' && (
          <div className="bg-red-50 text-red-700 p-4 rounded-xl flex flex-col items-center gap-2">
            <p className="font-semibold">Sai rồi, đáp án đúng là:</p>
            <p className="text-3xl font-black">{currentQ.term}</p>
            <p className="text-gray-600">Nghĩa: {currentQ.meaningVi}</p>
          </div>
        )}
      </div>
    </div>
